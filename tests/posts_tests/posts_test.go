...
func TestPost(t *testing.T) {
	...
	txt := "Test post"
	res, err := client.CreatePost(context.Background(), &pb.CreateRequest{AuthorId: 1, Text: txt})
	assert.NoError(t, err)
	postId := res.PostId

	req := &pb.PostIdRequest{AuthorId: 1, PostId: postId}
	...
	page, err := client.GetPageOfPosts(context.Background(),
		&pb.GetPageOfPostsRequest{AuthorId: 1, PageNum: 1, PageSize: 6})
	assert.NoError(t, err)
	...
	for _, id := range ids {
		_, err = client.DeletePost(context.Background(), &pb.PostIdRequest{AuthorId: 1, PostId: id})
		assert.NoError(t, err)
	}
}
