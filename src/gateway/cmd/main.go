...
func main() {
	...
	statsC, err := grpc.NewClient(
		os.Getenv("STATISTICS_SERVER_ADDR"),
		grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		log.Fatalf("did not connect: %v", err)
	}
	...
	services := service.NewService(db, postsC, statsC, p, kafkaConfig, kafkaEventCh)
