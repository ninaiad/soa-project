...
func (a *AuthPostgres) CreateUser(user main_service.User) error {
	var id int
...
func (a *AuthPostgres) GetUser(username, password string) (main_service.User, error) {
	var user main_service.User
	query := fmt.Sprintf("SELECT id FROM %s WHERE username=$1 AND password_hash=$2", usersTable)
	err := a.db.Get(&user, query, username, password)
	return user, err
}

func (a *AuthPostgres) GetUserData(userId int) (main_service.UserPublic, error) {
	user := main_service.UserPublic{}
	query := fmt.Sprintf("SELECT name, surname, birthday, email, phone FROM %s WHERE id = $1", usersTable)
	err := a.db.Get(&user, query, userId)
	return user, err
}

func (a *AuthPostgres) UpdateUser(userId int, update main_service.UserPublic, timeUpdated string) error {
